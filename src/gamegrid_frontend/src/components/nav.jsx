...
const  Nav =({setActivateSignIn}) => {
  const {login,logout,isAuthenticated,BackendActor,principal}=useAuth();
    ...
    return (
     ...
     {  showNav && <div className=' lg:hidden bg-gray-900 backdrop-blur-2xl z-50 border-r border-r-orange-900 w-full h-[100%] fixed'>
      ...
      <ul className="mx-5 text-white mt-5 flex  flex-col gap-5 text-xl  justify-center">
        ...
        <Link to="*" className="hover:text-2xl transition-all ease-in p-1 px-5 rounded border border-gray-500 cursor-pointer flex gap-1 items-center text-gray-400"><MdOutlineLeaderboard/>Leaderboard <span className="italic text-sm text-orange-600 ml-5">Locked</span></Link>
        <Link to="*" className="hover:text-2xl transition-all ease-in p-1 px-5 rounded border border-gray-500 cursor-pointer flex gap-1 items-center text-gray-400"><IoBag />Marketplace<span className="italic text-sm text-orange-600 ml-5">Locked</span></Link>
        {/* Principal */}
     { isAuthenticated &&
      <div className=" bg-gray-700 p-1 px-5 self-center rounded-full mt-64">{principal && principal.toText().slice(0, 5)}...{principal && principal.toText().slice(-3)}
        </div>}
        {/* Authentication */}
            { isAuthenticated? <button onClick={()=>{//clear();
      ...
      <ul className="mx-5 text-white mt-5 flex  flex-col gap-5 text-xl  justify-center">
        ...
        <Link to="/games" className={`hover:text-2xl transition-all ease-in p-1 ${currentPage ==="games" && "bg-gray-800"} px-5 rounded border border-gray-500 cursor-pointer flex gap-1 items-center `}><MdOutlineGames/>Games</Link>
        <Link to="*" className={`hover:text-2xl transition-all ease-in p-1 ${currentPage ==="profile" && "bg-gray-800"} px-5 rounded border border-gray-500 cursor-pointer flex gap-1 items-center text-gray-400`}><GiPlagueDoctorProfile />Profile <span className="italic text-sm text-orange-600 ml-5">Locked</span></Link>  <Link to="*" className="hover:text-2xl transition-all ease-in p-1 px-5 rounded border border-gray-500 cursor-pointer flex gap-1 items-center text-gray-400"><MdOutlineLeaderboard/>Leaderboard <span className="italic text-sm text-orange-600 ml-5">Locked</span></Link>
        <Link to="*" className="hover:text-2xl transition-all ease-in p-1 px-5 rounded border border-gray-500 cursor-pointer flex gap-1 items-center text-gray-400"><IoBag />Marketplace<span className="italic text-sm text-orange-600 ml-5">Locked</span></Link>
        {/* Principal */}
     { isAuthenticated &&
     ...
     {principal && principal.toText().slice(0, 5)}...{principal && principal.toText().slice(-3)}
        ...
        {/* Authentication */}
            {isAuthenticated?<button onClick={()=>{logout();setActivateSignIn(false);}} className="px-5 py-1  md:px-8 bg-orange-700 text-white mb-12">Logout</button>:<button onClick={()=>{setActivateSignIn(true);}} className="px-5 py-1  md:px-8 bg-orange-600 text-white mb-12">Sign In</button>}
